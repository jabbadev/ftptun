...
var Downloader = function( conf, url ) {
	...
	this.startDownload = function(resSize){
		...
		var outStream = fs.createWriteStream('prova.tar.bz2');
		...
		var hdOpt = { reqOpt: this.reqOpt, ptun: this.conf.ptun };
		//var hdOpt = { reqOpt: this.reqOpt };
		
		var hd = new HttpDownloader(hdOpt);
		...
		hd.on('end',function(data){
			...
			//console.log('xxxxxxxxxxx: ',secMsg);
