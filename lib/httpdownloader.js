...
var HttpDownloader = function(config){
	...
	function onDataHandler(data){
		var dataB64,dataSize = null, chunkInfo = null;
		if ( config.ptun ) {
			dataB64 = decipher.update(data);
			data = new Buffer(dataB64,'base64');
		}
		dataSize = (data) && data.length ;
		chunkInfo = ( config.chunk ) && config.chunk;
		
		self.emit('data',data,{ url: url, resSize: resSize, dataSize: dataSize, resChunkSize: resChunkSize, chunk: chunkInfo });
	};
	
	function onFinishHandler(){
		var data = null, dataSize = null, chunkInfo = null;
		if ( config.ptun ) {
		...
		self.emit('end',data,{ url: url, resSize: resSize, dataSize: dataSize, resChunkSize: resChunkSize, chunk: chunkInfo});
	};
		...
		request(reqOpt,function(error, response, body){
			if (!error && response.statusCode == 200) {
				...
				if(!config.chunk){
					...
					resChunkSize = response.headers['content-length'];	
					(isNaN(Number(resChunkSize))) ? resChunkSize = null : resChunkSize = parseInt(resChunkSize);
				}
			}
		}).pipe(stream);
	}
